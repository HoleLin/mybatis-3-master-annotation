...
public class JavassistProxyFactory implements org.apache.ibatis.executor.loader.ProxyFactory {
  ...
  private static class EnhancedResultObjectProxyImpl implements MethodHandler {
    ...
    public Object invoke(Object enhanced, Method method, Method methodProxy, Object[] args) throws Throwable {
      ...
      try {
        // 首先会在 loaderMap 集合上加锁防止并发，然后通过 lazyLoader 集合的长度，判断是否存在延迟加载的属性
        synchronized (lazyLoader) {
          if (WRITE_REPLACE_METHOD.equals(methodName)) {
            ...
            if (lazyLoader.size() > 0 && !FINALIZE_METHOD.equals(methodName)) {
              // 首先，会优先检查全局的 aggressive 配置和 lazyLoadTriggerMethods 配置。如果 aggressive 配置为 true，
              // 或此次调用方法名称包含于 lazyLoadTriggerMethods 配置的方法名列表中，会立刻将该对象的全部延迟加载属性都加载上来，即触发 ResultLoaderMap.loadAll() 方法。
